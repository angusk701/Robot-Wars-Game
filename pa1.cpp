                                   ...
                                   int &targetUpdatedHealthPoint)
{
    ...
    int a = -1;
    int b = -1;
    int& robot_vertical = a;
    int& robot_horizontal = b;
    ...
    if (directionLetter == DIRECTION_NORTH){
        if (map[robot_horizontal-1][robot_vertical] == CHAR_EMPTY){
            return STATUS_ACTION_WEAPON_FAIL;
        }
        else if (robot_horizontal - 1 <0)
        {
            return STATUS_ACTION_WEAPON_FAIL;
        }
        else{
            return STATUS_ACTION_WEAPON_SUCCESS;
            targetRobotLetter = map[robot_horizontal-1][robot_vertical];
            ...
            if (targetUpdatedHealthPoint <= 0){
                ...
                map[robot_horizontal-1][robot_vertical] = CHAR_EMPTY;
            }
            else {
    ...
    if (directionLetter == DIRECTION_SOUTH){
        ...
        else if (robot_horizontal + 1 < mapRows)
        {
            return STATUS_ACTION_WEAPON_FAIL;
        }
        else{
    ...
    if (directionLetter == DIRECTION_EAST){
        if (map[robot_horizontal][robot_vertical + 1] == CHAR_EMPTY){
            return STATUS_ACTION_WEAPON_FAIL;
        }
        else if (robot_vertical + 1 > mapCols)
        {
            return STATUS_ACTION_WEAPON_FAIL;
        }
        else{
            return STATUS_ACTION_WEAPON_SUCCESS;
            targetRobotLetter = map[robot_horizontal][robot_vertical + 1];
            targetOriginalHealthPoint = healthPoints[robotLetterToArrayIndex(targetRobotLetter)];
            targetUpdatedHealthPoint = targetOriginalHealthPoint - WEAPON_HIT_DAMAGE;
            if (targetUpdatedHealthPoint <= 0){
                targetUpdatedHealthPoint = 0;
                map[robot_horizontal][robot_vertical + 1] = CHAR_EMPTY;
            }
            else {
                healthPoints[robotLetterToArrayIndex(targetRobotLetter)] = targetUpdatedHealthPoint;
            }
                                     ...
                                     int &targetUpdatedHealthPoint)
{
    ...
    if (directionLetter == DIRECTION_NORTH){
        int targetRobotPosition = -1;
        int targetRobotNumber = 0;
        int empty_space = 0;
        int distance_from_top = robot_horizontal - WEAPON_SHOOT_RANGE;
        if (distance_from_top < 0){
            return STATUS_ACTION_WEAPON_FAIL;
        }
        else{
            for (int i = robot_horizontal; i >= distance_from_top; i--){
                if (map[i][robot_vertical] != CHAR_EMPTY){
                    if (targetRobotNumber == 0){
                        return STATUS_ACTION_WEAPON_SUCCESS;
                        targetRobotPosition = i;
                        targetRobotLetter = map[targetRobotPosition][robot_vertical];
                        targetOriginalHealthPoint = healthPoints[robotLetterToArrayIndex(targetRobotLetter)];
                        targetUpdatedHealthPoint = targetOriginalHealthPoint - WEAPON_SHOOT_DAMAGE;
                        if (targetUpdatedHealthPoint <= 0){
                            targetUpdatedHealthPoint = 0;
                            map[targetRobotPosition][robot_vertical] = CHAR_EMPTY;
                        }
                        else {
                            healthPoints[robotLetterToArrayIndex(targetRobotLetter)] = targetUpdatedHealthPoint;
                        }
    ...
    if (directionLetter == DIRECTION_WEST){
        ...
        int targetRobotNumber = 0;
        int empty_space = 0;
        int final_vertical = robot_vertical - WEAPON_SHOOT_RANGE;
        if (final_vertical < 0){
            return STATUS_ACTION_WEAPON_FAIL;
        }
        else{
            for (int i = robot_vertical; i >= final_vertical; i--){
                if (map[robot_horizontal][i] != CHAR_EMPTY){
                    if (targetRobotNumber == 0){
                        return STATUS_ACTION_WEAPON_SUCCESS;
                        targetRobotPosition = i;
                        targetRobotLetter = map[robot_horizontal][i];
                        targetOriginalHealthPoint = healthPoints[robotLetterToArrayIndex(targetRobotLetter)];
                        targetUpdatedHealthPoint = targetOriginalHealthPoint - WEAPON_SHOOT_DAMAGE;
                        if (targetUpdatedHealthPoint <= 0){
                            targetUpdatedHealthPoint = 0;
                           ...
                           const char robotLetter, const char directionLetter, const int moveSteps)
{
    ...
    int a = -1;
    int b = -1;
    int& initial_vertical = a;
    int& initial_horizontal = b;
    for (int i = 0; i<= mapRows; i++ ){
        for (int j = 0; j <= mapCols; j++){
            if (map[i][j] == robotLetter){
                a = j;
    ...
    if (directionLetter == 'n'){
        int distance_from_top = initial_horizontal - moveSteps;
        if (distance_from_top < 0){
            return STATUS_ACTION_MOVE_OUTSIDE_BOUNDARY;
        }
        else{
            for (int i = initial_horizontal; i >= distance_from_top; i--){
                if (map[i][initial_vertical] != CHAR_EMPTY){
                    not_move += 1;
                }
            }
            // robot moves if not_move == 0
            if (not_move != 0){
                return STATUS_ACTION_MOVE_HIT_ANOTHER_ROBOT_ALONG_PATH;
                not_move = 0;
            }
            else{
                map[initial_horizontal][initial_vertical] = CHAR_EMPTY;
                map[distance_from_top][initial_vertical] = robotLetter;
                return STATUS_ACTION_MOVE_SUCCESS;
            }
        }
    }
    if (directionLetter == 's'){
        int final_horizontal = initial_horizontal + moveSteps;
        if (final_horizontal > mapRows){
            return STATUS_ACTION_MOVE_OUTSIDE_BOUNDARY;
        }
        else{
            for (int i = initial_horizontal; i <= final_horizontal; i++){
                if (map[i][initial_vertical] != CHAR_EMPTY){
                    not_move = not_move+1;
                }
            ...
            if (not_move != 0){
                ...
                map[initial_horizontal][initial_vertical] = CHAR_EMPTY;
                map[final_horizontal][initial_vertical] = robotLetter;
                return STATUS_ACTION_MOVE_SUCCESS;
            }
        }
    }
    if (directionLetter == 'w'){
        int final_vertical = initial_vertical - moveSteps;
        if (final_vertical < 0){
            return STATUS_ACTION_MOVE_OUTSIDE_BOUNDARY;
        }
        else{
            for (int i = initial_vertical; i >= final_vertical; i--){
                if (map[i][initial_vertical] != CHAR_EMPTY){
                    not_move += 1;
                }
            ...
            if (not_move != 0){
                ...
                map[initial_horizontal][initial_vertical] = CHAR_EMPTY;
                map[initial_horizontal][final_vertical] = robotLetter;
                return STATUS_ACTION_MOVE_SUCCESS;
            }
        }
    }
